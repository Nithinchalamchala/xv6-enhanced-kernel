...
sys_write(void)
{
  ...
  if(f->type == FD_INODE && f->ip){
    ...
    if(!(f->ip->permissions & PERM_WRITE)){
      cprintf("Permission denied: file is read-only\n");
    ...
    if(f->ip->uid != 0 && f->ip->uid != curproc->uid){
      if(!(curproc->permissions & PERM_WRITE)){
        cprintf("Permission denied: user has no write permission\n");
...
create(char *path, short type, short major, short minor)
{
  ...
  ip->nlink = 1;
  ip->uid = myproc()->uid;
  ip->permissions = PERM_READ | PERM_WRITE;  // Default permissions
  iupdate(ip);
...
sys_open(void)
{
  ...
  // Check write permission before opening for write
  if((omode & O_WRONLY) || (omode & O_RDWR)){
    if(!(ip->permissions & PERM_WRITE)){
      cprintf("Permission denied: cannot open read-only file for writing\n");
      iunlockput(ip);
      end_op();
      return -1;
    }
    ...
    struct proc *curproc = myproc();
    if(ip->uid != 0 && ip->uid != curproc->uid){
      if(!(curproc->permissions & PERM_WRITE)){
        cprintf("Permission denied: user has no write permission\n");
        iunlockput(ip);
        end_op();
